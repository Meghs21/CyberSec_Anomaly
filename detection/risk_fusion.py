"""
Risk Fusion & Dynamic Smart Thresholding Engine.
Fuses signals from:
1. Behavioral baseline deviation
2. Sequence anomaly model (Deliverable #3)
3. Rule assist engine
into a unified risk_score in [0.0, 100.0].
...
class RiskFusionEngine:
    ...
    def fuse_risk_score(self, event, rule_signals, ml_raw_score, sequence_score, baseline_stats):
        """
        Combines rule overrides, sequence anomaly score, and ML raw score into risk_score [0, 100].
        ...
        # Base score from ML anomaly model & sequence transition model (Deliverable #3)
        base_risk = min(50.0, max(0.0, ml_raw_score * 20.0)) + (sequence_score * 30.0)
