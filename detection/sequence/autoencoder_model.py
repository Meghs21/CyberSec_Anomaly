"""
Sequence Intelligence Subsystem: Behavioral Autoencoder Model.
Implements a Feedforward Neural Autoencoder over fixed-length behavioral windows (K=5 events x 6 features = 30-dim input).

Architecture:
  Input (30 features) -> Dense(16) -> Latent Bottleneck (8-dim behavior embedding) -> Dense(16) -> Output (30 features)

Rationale:
  The 8-dimensional bottleneck forces compression of normal behavioral patterns into a compact latent space.
  Unusual sequences produce high reconstruction MSE error, indicating behavior outside the learned latent manifold.
...
class SequenceAutoencoderDetector:
    def __init__(self, hidden_layer_sizes=(16, 8, 16), max_iter=100, random_state=42):
        ...
        # 30 -> 16 -> 8 (latent bottleneck) -> 16 -> 30 autoencoder
    ...
    def _extract_event_vector(self, event):
        ...
        assert "label" not in event, "LABEL LEAKAGE DETECTED: Ground-truth 'label' must be removed before feature extraction!"
    ...
    def fit_normal_baseline(self, events):
        """Fits neural autoencoder strictly on normal baseline event windows."""
