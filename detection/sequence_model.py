"""
Sequence-Aware Anomaly Detection Engine (Deliverable #3).
Implements N-gram Markov transition probability model over resource_accessed and command_sequence.
Includes Laplace additive smoothing, cohort transition fallback for cold-start entities,
and anti-poisoning online updates.
"""
...
class SequenceAnomalyDetector:
    ...
    def _extract_states(self, event):
        ...
        # Combine resource and command tokens into ordered sequence
    ...
    def fit_normal_baseline(self, events):
        ...
        for ev in events:
            # Drop label if present in training split
            assert "label" not in ev or ev.get("label") == "normal", "Sequence baseline model training on normal events"
    ...
    def calculate_sequence_score(self, event, entity_profile):
        """
        Calculates sequence anomaly score in [0.0, 1.0].
        0.0 = expected transition sequence; 1.0 = highly anomalous transition sequence.
        Fails loudly if label leakage is detected!
        """
        assert "label" not in event, "LABEL LEAKAGE DETECTED: Ground-truth 'label' field must be removed before sequence scoring!"
        ...
        # Calibrate mean -log(P) to [0, 1] scale (typical range 0.5 to 11.5)
